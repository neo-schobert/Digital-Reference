...
import { getFullGraph, getMeta, PREFIXES, store } from "./ontology.js";
...
function computeMembers(req: SplitRequest): Set<string> {
...
export function buildSplit(req: SplitRequest): { filename: string; ttl: string } {
  ...
  const members = computeMembers({ ...req, seeds });
  ...
  const meta = getMeta();
  const name = (req.name ?? "").trim() || "DR split";
  const slug =
    name
      .toLowerCase()
      .replace(/[^a-z0-9]+/g, "-")
  ...
  const header =
    `<http://www.w3id.org/ecsel-dr/split/${slug}> a owl:Ontology ;\n` +
    `  rdfs:label "${escStr(name)} — Digital Reference split"@en ;\n` +
    `  rdfs:comment "${escStr(
      `Standalone structural split of the Digital Reference, generated by DR Explorer. ` +
        `Seeds: ${seedLabels}. Expansion: ${rules}. ` +
        `${members.size} classes, ${objProps.size} object properties, ${dtProps.size} datatype properties.`
    )}"@en ;\n` +
    `  owl:versionInfo "split of Digital Reference ${escStr(meta.ontology.version)}" ;\n` +
    `  terms:source <http://www.w3id.org/ecsel-dr> ;\n` +
  ...
  const ttl =
    `# ${name} — structural split of the Digital Reference (${meta.ontology.version})\n` +
    `# Generated by DR Explorer on ${now}\n\n` +
    `${prefixHeader}\n\n${header}\n` +
